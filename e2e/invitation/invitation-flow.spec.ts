...
import { generateInvitationCode, joinGroupWithInvitation } from "../helpers/invitation";
...
test.describe("招待機能のE2Eフロー", () => {
	...
	test("無効な招待コードでエラーが表示される", async ({ page }) => {
		...
		// エラーメッセージが表示されることを確認
		await expect(
			page.locator("text=招待コードが無効です"),
		).toBeVisible();
	});

	test.describe("フルフロー統合テスト", () => {
		test.skip("招待コード生成→共有→新規ユーザー参加のフルフロー", async ({ page, context }) => {
		...
		test.skip("招待一覧にリアルタイムで新規招待が表示される", async ({ page }) => {
	...
	test.describe("Patient制約のUI表示", () => {
		test.skip("Patient存在グループでの招待生成はSupporterのみ表示", async ({ page }) => {
		...
		test.skip("グループメンバー一覧でPatientバッジが表示される", async ({ page }) => {
	...
	test.describe("エラーハンドリングUI", () => {
		test("有効期限切れ招待コードでエラーメッセージが表示される", async ({ page }) => {
		...
		test.skip("重複参加時にエラーメッセージとリダイレクトが発生する", async ({ page }) => {
		...
		test.skip("Patient重複エラーでロール変更ガイダンスが表示される", async ({ page }) => {
			// NOTE: このテストは認証とPatient存在グループのセットアップが必要なためスキップ
	...
	test.describe("招待リンク共有機能", () => {
		test.skip("クリップボードコピー機能が動作する", async ({ page, context }) => {
			// NOTE: このテストは認証とグループセットアップが必要なためスキップ
		...
		test.skip("モバイル環境でWeb Share APIボタンが表示される", async ({ page }) => {
