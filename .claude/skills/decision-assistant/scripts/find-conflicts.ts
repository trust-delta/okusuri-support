...
const CONFLICT_PATTERNS = [
	{ positive: ["採用", "使用", "導入", "移行"], negative: ["廃止", "削除", "除外", "非推奨"] },
	{ positive: ["許可", "有効化", "enable"], negative: ["禁止", "無効化", "disable"] },
...
function isSameCategory(
	...
	for (const [category, keywords] of Object.entries(TECHNOLOGY_CATEGORIES)) {
		...
		const existingHasCategory = keywords.some((catKw) => lowerContent.includes(catKw));
...
function detectConflictPattern(
	...
	for (const pattern of CONFLICT_PATTERNS) {
		...
		const existingHasNegative = pattern.negative.some((n) => lowerContent.includes(n));
		...
		const existingHasPositive = pattern.positive.some((p) => lowerContent.includes(p));
 ...
 * 競合する決定記録を検出
 */
export function findConflicts(newDecisionKeywords: string[]): ConflictingDecision[] {
	...
	for (const filePath of files) {
		...
		if (!metadata.status.includes("承認済み") && !metadata.status.includes("実装完了")) {
			continue;
		}
		...
		// 2. 競合パターンを検出
		const patternCheck = detectConflictPattern(newDecisionKeywords, metadata.content);
...
if (require.main === module) {
	...
	console.log(`⚠️  Found ${results.length} potentially conflicting decision(s):\n`);

	for (const result of results) {
		const riskEmoji = result.riskLevel === 3 ? "🔴" : result.riskLevel === 2 ? "🟡" : "🟢";
