...
describe("招待機能 - 統合テスト", () => {
	describe("招待コード生成から参加までのエンドツーエンドフロー", () => {
		it("グループメンバーAが招待コードを生成し、ユーザーBが参加できる", async () => {
			...
			const validation = await t.query(api.invitations.queries.validateInvitationCode, {
				code: "INTEGRATION",
			});
		...
		it("招待一覧にリアルタイムで新しい招待が表示される", async () => {
			...
			// 最初は招待が0件
			let invitations = await asUser.query(api.invitations.queries.listGroupInvitations, {
				groupId,
			});
			...
			invitations = await asUser.query(api.invitations.queries.listGroupInvitations, {
				groupId,
			});
	...
	describe("Patient制約の統合テスト", () => {
		...
		it("Supporterロール選択時は既存Patientの有無に関わらず参加できる", async () => {
			...
			const result = await asNewUser.mutation(api.groups.mutations.joinGroupWithInvitation, {
	...
	describe("有効期限切れ招待の処理テスト", () => {
		...
		it("有効期限内の招待コードは正常に機能する", async () => {
			...
			const result = await asUser.mutation(api.groups.mutations.joinGroupWithInvitation, {
	...
	describe("並行招待使用の競合処理テスト", () => {
		it("同一招待コードを2人が同時に使用した場合、1人目のみ成功する", async () => {
			...
			// 1人目が参加
			const result1 = await asUser1.mutation(api.groups.mutations.joinGroupWithInvitation, {
		...
		it("異なる招待コードを使用した並行参加は両方成功する", async () => {
			...
			const result2 = await asUser2.mutation(api.groups.mutations.joinGroupWithInvitation, {
