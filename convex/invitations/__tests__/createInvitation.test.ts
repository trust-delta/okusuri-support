...
describe("createInvitationInternal - 招待コード生成ロジック", () => {
	describe("認証とメンバーシップ検証", () => {
		...
		it("グループメンバーでない場合はエラーを返す", async () => {
			...
			await expect(
				asNonMember.mutation(api.invitations.mutations.createInvitationInternal, {
					groupId,
					code: "TEST1234",
				}),
	...
	describe("Patient在籍状況に基づくロール制御", () => {
		...
		const asUser = t.withIdentity({ subject: userId });
			...
			// 招待コードを生成
			const result = await asUser.mutation(api.invitations.mutations.createInvitationInternal, {
				groupId,
				code: "TEST1234",
			});
