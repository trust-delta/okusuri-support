...
- [ ] 14. バックエンドのユニットテストを作成
- [ ] 14.1 招待コード生成ロジックのテスト
  - Patient不在時の両ロール許可検証
...
- [ ] 15. 統合テストの実装
- [ ] 15.1 招待コード生成から参加までのエンドツーエンドフローをテスト
  - グループメンバーAによる招待コード生成
  - ユーザーBによる招待コードでの参加
...
- [ ] 16. E2Eテストの実装
- [ ] 16.1 招待リンク共有からグループ参加までのフルフローテスト
  - 招待コード生成UI操作
  - 招待リンクコピーまたは共有
  - 新規ユーザーによるリンクアクセス
  - ロール選択と表示名入力
...
- [ ] 17. パフォーマンステストの実施
- [ ] 17.1 招待コード生成のレイテンシ測定
  - Patient存在チェッククエリの実行時間測定
  - 目標: 500ms以下での応答確認
  - _Requirements: 1.1, 1.2, 1.3_

- [ ] 17.2 大量招待の一覧表示パフォーマンス確認
...
- [ ] 19. セキュリティ検証
- [ ] 19.1 招待コードの総当たり攻撃耐性確認
  - 8文字英数字の組み合わせ数（約218兆通り）の十分性確認
  - 有効期限7日間の妥当性確認
  - レートリミット実装の検討（将来課題）
  - _Requirements: 6.4_
...
- [ ] 20.2 WAI-ARIA準拠とスクリーンリーダー対応の確認
